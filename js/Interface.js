...
SancCity.storeLeaveButton = false;
...
//         });
//     }
    let andreaShop = false;


    SancCity.interface.populateShop = function(shop_object){
        if(SancCity.storeLeaveButton===false){ 
            SancCity.storeLeaveButton ===true
            }
            SancCity.session.pause()
       

        document.getElementById('hidden-store').classList.remove('hidden')
        document.getElementById('leave_btn').addEventListener('click', function() {
            SancCity.session.resume();
        ...
        let this_purchase=0;
            for (i=0;i<purchase.length;i++){
                ...
                if (purchase[i].value>0)
                    { quantity = i
                        if (purchase[i].value > document.getElementsByClassName('item_'+num+"_quantity")[quantity].innerText){
                            this_purchase  =  parseInt(document.getElementsByClassName('item_'+num+"_quantity")[quantity].innerText)
                        }
                     else this_purchase = parseInt(purchase[i].value);}
            }
            ...
            if(total > SancCity.convoy.money){
               ...
               SancCity
                console.log( document.getElementsByClassName('item_'+num+"_quantity")[quantity])

               document.getElementsByClassName('item_'+num+"_quantity")[quantity].innerText -= this_purchase;
